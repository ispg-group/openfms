"""Plot histogram of NHC momentum samples from nhc_momentum_beta=1.dat."""

import math
import os

data_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nhc_momentum_beta=1.dat")
...
ax.hist(data, bins=80, density=True, color="steelblue", alpha=0.6, label="NHC samples")
...
ax.set_title(f"NHC 1-D HO canonical momentum  ($N={N:,}$,  $m=\\beta=\\omega=1$)")
ax.legend()
fig.tight_layout()

out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nhc_momentum_beta=1.png")
