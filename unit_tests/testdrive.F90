...
module testdrive
  ...
  public :: init_color_output, get_final_message
...
contains
  ...
  recursive subroutine run_testsuite(collect, unit, stat, parallel, junit)
    ...
    do it = 1, size(testsuite)
      ...
      ! FMS-CHANGE: Make the output terser 
      write(unit, '(1x, 4(1x, a))', advance="no") (color%blue) // testsuite(it)%name // color%reset
  ...
  pure subroutine make_output(output, test, error)
    ...
    ! FMS-CHANGE: Terser output
    output = " "//color%bold//"["//label_color//label//color%bold//"]"//color%reset
    if (present(error)) then
      output = output // newline // "  "//color%bold//"Message:"//color%reset//" " // error%message
    end if
  end subroutine make_output

  pure function get_final_message(num_failed_tests) result(output)

    !> Number of failed tests
    integer, intent(in) :: num_failed_tests
    !> Output message for display
    ...
    if (num_failed_tests > 0) then
      ...
      if (num_failed_tests == 1) then
        write(output, '(a)') "1 test FAILED"
      else
        write(output, '(i0, 1x, a)') num_failed_tests, "tests FAILED"
      end if
    end if

    output = color%bold + label_color // trim(output) // color%reset

  end function get_final_message
  ...
  subroutine check_logical(error, expression, message, more)
    ...
    if (.not.expression) then
      if (present(message)) then
        ...
        call test_failed(error, "Condition not fulfilled", more)
      end if
    end if
  ...
  subroutine check_float_dp(error, actual, expected, message, more, thr, rel)
    ...
    if (diff > threshold) then
      if (present(message)) then
        ...
        if (relative) then
          call comparison_failed(error, "Floating point relative", &
            expected=to_string(expected), &
            actual=to_string(actual)//" (difference: "//to_string(int(diff*100))//"%)", &
            more=more)
        else
          call comparison_failed(error, "Floating point", &
            expected=to_string(expected), &
            actual=to_string(actual) // " (difference: "//to_string(diff)//")", &
            more=more)
        end if
      end if
    end if
  ...
  subroutine check_int_i1(error, actual, expected, message, more)
    ...
    if (expected /= actual) then
      if (present(message)) then
        ...
        call comparison_failed(error, "Integer", &
  ...
  subroutine check_bool(error, actual, expected, message, more)
    ...
    if (expected .neqv. actual) then
      if (present(message)) then
        ...
        call comparison_failed(error, "Logical", &
          expected=merge("T", "F", expected), &
          actual=merge("T", "F", actual), &
          more=more)
      end if
    end if
  ...
  subroutine check_string(error, actual, expected, message, more)
    ...
    if (expected /= actual) then
      if (present(message)) then
        ...
        call comparison_failed(error, "String", expected, actual, more)
      end if
    end if

  end subroutine check_string

  ! A simple wrapper common for all types
  subroutine comparison_failed(error, type, expected, actual, more)
    !> Error handling
    type(error_type), allocatable, intent(out) :: error
    ...
    !> Found integer value (stringified)
    character(len=*), intent(in) :: actual

    !> Expected integer value (stringified)
    character(len=*), intent(in) :: expected
    ...
    message = message // skip // color%dim // 'expected: ' // color%reset // expected
    message = message // skip // color%dim // '     got: ' // color%reset // actual

    call test_failed(error, message, more)
  end subroutine comparison_failed
