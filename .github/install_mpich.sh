...
if [[ -z ${1-} ]]; then
  echo "ERROR: Provide path to the installation directory as a first parameter"
  echo "$0 installation_path [mpich_version]"
  exit 1
fi
...
# By default, all available CPUs are used. You can override it here
NCPUS=""
FC=${FC:-gfortran}
CC=${CC:-gcc}
...
echo "Succesfully installed MPICH-${MPICH_VERSION}!"
echo "Add '${INSTALL_DIR}/bin' to your PATH to use it."
