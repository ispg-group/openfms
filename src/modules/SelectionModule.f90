...
module SelectionModule
   ...
   use BundleCalcsModule, only: FMS_bH, FMS_Norm, FMS_Mulliken
   use OverlapModule, only: overlap
   use RandomModule, only: fms_ranb
   use SpawnModule, only: spawn_params, spawn_couple
   implicit none

   private
   public :: print_stochastic_selection_params
   public :: FMS_StochasticCollapse, FMS_RemoveDead
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   subroutine FMS_BuildCoupled_RemoveDead(B1, Coupled)
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      type(T_TrajectoryBundle), intent(in) :: B1
      integer(kind=DefInt), dimension(B1%NumTraj, B1%NumTraj), &
         intent(inout) :: Coupled
...
! Uncover all indirect connections between TBFs
   ...
   end subroutine FMS_BuildCoupled_RemoveDead

   subroutine FMS_Count_Nr_Dead(B1, Coupled, nr_dead)
      type(T_TrajectoryBundle), intent(inout) :: B1
      integer(kind=DefInt), dimension(B1%NumTraj, B1%NumTraj), &
         intent(inout) :: Coupled
      integer(kind=DefInt), intent(out) :: nr_dead
      ...
      integer(DefInt) :: ntraj, i, TrajID, StateID, nstate, n
   ...
   end subroutine FMS_Count_Nr_Dead
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   subroutine FMS_RemoveDead(B1)
      ...
      integer(DefInt), dimension(B1%NumTraj, B1%NumTraj) :: Coupled

      integer(DefInt) :: ntraj, i

      ntraj = B1%NumTraj

      Coupled = 0

      call FMS_BuildCoupled_RemoveDead(B1, Coupled)

      do i = 1, ntraj
         Coupled(i, i) = 0
      end do

      call FMS_Count_Nr_Dead(B1, Coupled, ndead)
